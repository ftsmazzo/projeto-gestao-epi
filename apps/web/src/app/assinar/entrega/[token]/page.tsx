...
import {
  completePortalDeliverySign,
  unlockPortalDeliverySign,
} from '../../../../lib/portal-delivery-sign-public';

type GuideTone = 'neutral' | 'adjusting' | 'ready' | 'processing' | 'matched';
...
export default function EpiDeliverySignPage() {
  ...
  const [workerFirstName, setWorkerFirstName] = useState<string | null>(null);
  ...
  async function onUnlock(event: React.FormEvent) {
    ...
    setUnlocking(true);
    try {
      const result = await unlockPortalDeliverySign(token, cpfLast4);
      setWorkerFirstName(result.workerFirstName);
  ...
  async function startScan() {
    if (!accepted) {
      setError('Confirme a ciencia para continuar.');
      return;
    }
    if (!modelsReady) {
  ...
  async function scanLoop() {
    while (scanningRef.current) {
      ...
      try {
        ...
        if (current === 'scanning') {
          ...
          if (hint === 'ready') {
            ...
            else if (Date.now() - stableSinceRef.current >= AUTO_CAPTURE_STABLE_MS && !capturingRef.current) {
          ...
          const tracker = livenessRef.current ?? createLivenessTracker();
      ...
      await new Promise((r) =>
        setTimeout(r, phaseRef.current === 'liveness' ? LIVENESS_INTERVAL_MS : SCAN_INTERVAL_MS),
      );
    }
  }
  ...
  async function captureAndSubmit(challenge: LivenessChallengeType) {
    ...
    try {
      ...
      await completePortalDeliverySign(token, blob, {
        cpfLast4,
    ...
    <main className={`enroll-page${live ? ' enroll-page--capture enroll-page--live' : ''}`}>
      ...
      <div className="enroll-page__sheet">
        <header className="enroll-page__hero">
          ...
          <h1 className="enroll-page__title">Assinatura da entrega de EPI</h1>
          <p className="enroll-page__lead">
            {phase === 'done'
              ? 'Assinatura registrada com sucesso.'
              : phase === 'locked'
                ? 'Informe os 4 ultimos digitos do CPF para continuar.'
                : `Ola${workerFirstName ? `, ${workerFirstName}` : ''}. Confirme com sua face.`}
          </p>
        </header>
        ...
        {phase === 'locked' ? (
          <form className="enroll-page__form" onSubmit={(e) => void onUnlock(e)}>
            ...
            >
              {unlocking ? 'Validando…' : 'Abrir assinatura'}
            </button>
          </form>
        ) : null}
        ...
        {phase === 'reading' ? (
          <div className="enroll-page__form">
            <p className="enroll-page__hint">{consentText ?? 'Confirme sua assinatura da entrega de EPI.'}</p>
            <label className="enroll-page__consent">
              ...
              />
              <span>Li e concordo com o registro da minha assinatura facial.</span>
            </label>
            <button
              type="button"
              className="enroll-page__btn"
        ...
        {phase === 'done' ? (
          <div className="enroll-page__success" role="status">
            <p>Assinatura concluida. Voce pode fechar esta pagina.</p>
          </div>
        ) : null}
